import { Award, BookOpen, Crown, Medal, MessageSquare, TrendingUp, Zap } from 'lucide-react';
    ...
    <div ref={staggerRef} className="mx-auto max-w-5xl space-y-8">
      <section data-stagger>
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(135deg, rgba(13,20,32,0.95) 0%, rgba(11,16,20,0.98) 60%, rgba(15,24,12,0.95) 100%)',
            border: '1px solid rgba(201,162,39,0.25)',
            boxShadow:
              '0 0 0 1px rgba(201,162,39,0.08), 0 20px 60px rgba(0,0,0,0.5), 0 0 80px rgba(201,162,39,0.06)',
          }}
        >
      ...
      {isLoading ? (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, i) => (
            <StatCard key={i} icon={<BookOpen className="h-5 w-5" />} label="" value="" loading />
          ))}
        </section>
      ) : (
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div data-stagger>
            <StatCard
              ...
              {...(stats?.provaFinalPassed ? { trend: 'up' as const, trendValue: 'Aprovado' } : {})}
            />
          </div>
          <div data-stagger>
            <StatCard
      ...
      <section data-stagger aria-labelledby="course-overview-heading">
        <GlassCard depth="surface" hover className="p-6">
          ...
          {stats?.provaFinalPassed &&
            (stats.provaFinalScore ?? 0) >= MESA_PROPRIETARIA_MIN_SCORE && (
